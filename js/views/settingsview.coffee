...
define([
	...
	# Reads the selected settings on the view when they change and passes them to the model.
	...
	class SettingsView extends Backbone.View
		...
		# Tells the view which data to set on the model when the user has changed a settings.
		events: =>
		...
		# @returns {Object} A hash representing the selected charset for the password. 
		# @nosideeffects
		getChars: =>
			chars =
		...
		# @returns {number} The selected length for the password. Is in [7, 14, 21, 42].
		# @nosideeffects
		getLength: =>
