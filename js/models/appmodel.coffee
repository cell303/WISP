...
define([
  ...
  class AppModel extends Backbone.Model
    ...
    # Sets the events that can cause the app to request passwords from the 
    # sever.
    initialize: ->
    ...
    # Setting an empty string as weak password means that the app should be 
    # reset. If a non-empty string is set, a request to the server is sent.
    onChange: =>
      if @get('weak') is ''
        @lock()
      else
